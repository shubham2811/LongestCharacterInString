function getLongestWord(string) {
    ...
    //regex for removing non letter chars
    const regex = /[^a-zA-Z ]/g;
    const words = string.replace(regex, '').split(' ');
    let maxWordLength = 0;
    let maxVowelCount = 0;
    let longestWord = '';
  ...
  //iterate over each word
    for (const word of words) {
      ...
      const vowelCount = lowerCaseWord.replace(vowelRegex, '').length;
      
      //check if word has larger length or if length is similar check for vowels count
      const checkWordCriteria = lowerCaseWord.length > maxWordLength || (lowerCaseWord.length === maxWordLength && vowelCount > maxVowelCount)
      if (checkWordCriteria) {
        longestWord = word;
        maxWordLength = lowerCaseWord.length;
